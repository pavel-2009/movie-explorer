import dunePoster from '../assets/images/posters/dune.svg'
import interstellarPoster from '../assets/images/posters/interstellar.svg'
import matrixPoster from '../assets/images/posters/matrix.svg'
import bladeRunnerPoster from '../assets/images/posters/blade-runner.svg'
import continuePoster from '../assets/images/posters/continue.svg'

export const movies = [
  {
    slug: 'dune-part-two',
    title: 'Дюна: Часть вторая',
    rating: 8.7,
    imageUrl: dunePoster,
    backdrop: dunePoster,
    genre: 'Фантастика, приключения, драма',
    duration: '2ч 46м',
    description:
      'Пол Атрейдес объединяется с Чани и фрименами, чтобы пройти путь мести, силы и выбора, от которого зависит будущее целой вселенной.',
    actors: [
      {
        name: 'Тимоти Шаламе',
        photo: 'https://image.tmdb.org/t/p/w185/BE2sdjpgsa2rNTFa66f7upkaOP.jpg',
      },
      {
        name: 'Зендея',
        photo: 'https://image.tmdb.org/t/p/w185/3WdOloHpjtjL96uVOhFRRCcYSwq.jpg',
      },
      {
        name: 'Ребекка Фергюсон',
        photo: 'https://image.tmdb.org/t/p/w185/lJloTOheuQSirSLXNA3JHsrMNfH.jpg',
      },
    ],
  },
  {
    slug: 'interstellar',
    title: 'Интерстеллар',
    rating: 8.6,
    imageUrl: interstellarPoster,
    backdrop: interstellarPoster,
    genre: 'Научная фантастика, драма',
    duration: '2ч 49м',
    description:
      'Команда исследователей отправляется сквозь червоточину в поисках нового дома для человечества.',
    actors: [
      {
        name: 'Мэттью Макконахи',
        photo: 'https://image.tmdb.org/t/p/w185/2u8G0zuM93Q6He3fMzsTg1eWOqq.jpg',
      },
      {
        name: 'Энн Хэтэуэй',
        photo: 'https://image.tmdb.org/t/p/w185/3KwDvGCmRXu89nd8uLaSME2z9aB.jpg',
      },
    ],
  },
  {
    slug: 'the-matrix',
    title: 'Матрица',
    rating: 8.7,
    imageUrl: matrixPoster,
    backdrop: matrixPoster,
    genre: 'Боевик, фантастика',
    duration: '2ч 16м',
    description:
      'Нео узнает, что мир вокруг него — это симуляция, а его миссия — освободить человечество.',
    actors: [
      {
        name: 'Киану Ривз',
        photo: 'https://image.tmdb.org/t/p/w185/4D0PpLj8gqfZzU5rB0u0hX2Bf5R.jpg',
      },
      {
        name: 'Лоуренс Фишбёрн',
        photo: 'https://image.tmdb.org/t/p/w185/6n3vOtJQSBixp1Oe9MB1zW7LNfH.jpg',
      },
    ],
  },
  {
    slug: 'blade-runner-2049',
    title: 'Блэйд Раннер 2049',
    rating: 8.0,
    imageUrl: bladeRunnerPoster,
    backdrop: bladeRunnerPoster,
    genre: 'Научная фантастика, детектив',
    duration: '2ч 43м',
    description:
      'Новый охотник за репликантами раскрывает тайны прошлого и настоящего человечества.',
    actors: [
      {
        name: 'Райан Гослинг',
        photo: 'https://image.tmdb.org/t/p/w185/5W4w6rUIyRdSf5Uk85kBfO0iB9s.jpg',
      },
      {
        name: 'Харрисон Форд',
        photo: 'https://image.tmdb.org/t/p/w185/5m2W9v5AOfnD5rHIZd2v3vWnR7B.jpg',
      },
    ],
  },
]

export const continueWatching = [
  { title: 'Сериал 1', progress: 'Сезон 2, Серия 5', imageUrl: continuePoster, watchedPercentage: 50 },
  { title: 'Сериал 2', progress: 'Сезон 1, Серия 10', imageUrl: continuePoster, watchedPercentage: 80 },
  { title: 'Сериал 3', progress: 'Сезон 3, Серия 2', imageUrl: continuePoster, watchedPercentage: 30 },
]
